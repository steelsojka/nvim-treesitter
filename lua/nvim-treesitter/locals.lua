...
function M.get_scope_tree(node, bufnr)
  local current_scope = M.containing_scope(node, bufnr) or parsers.get_tree_root(bufnr)
  local scopes = {}

  while current_scope do
    table.insert(scopes, current_scope)
    current_scope = current_scope:parent()
      and (M.containing_scope(current_scope:parent(), bufnr) or parsers.get_tree_root(bufnr))
      or nil
  end
...
function M.find_definition(node, bufnr)
  ...
  for _, definition in ipairs(M.get_definitions(bufnr)) do
    for _, node_entry in ipairs(M.get_local_nodes(definition)) do
      local def_scope = M.containing_scope(node_entry.node, bufnr) or parsers.get_tree_root(bufnr)
...
function M.nested_scope(node, cursor_pos)
  ...
  local scope = M.containing_scope(node) or node
